...
Par mat_select_point(const matchar& mat, const string & texto){
    return MatrizView::instance()->select_point(mat, texto);
}

void mat_paint_grid(const matchar& mat){
    MatrizView::instance()->paint_target_squares(*MyPlayer::getPainel(), mat);
}

void mat_paint_nums(const matriz<int>& nums){
    MatrizView::instance()->paint_target_nums(*MyPlayer::getPainel(), nums);
}

void mat_paint_cell(const Par & pos, char color){
